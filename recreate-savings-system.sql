-- =====================================================
-- RECRÉATION COMPLÈTE DU SYSTÈME D'ÉPARGNE
-- =====================================================
...
CREATE TABLE public.savings_plans (
    ...
    total_deposits_required INTEGER GENERATED ALWAYS AS (
        (duration_months * 30) / frequency_days
    ) STORED,
    total_amount_target DECIMAL(15,2) GENERATED ALWAYS AS (
        fixed_amount * ((duration_months * 30) / frequency_days)
    ) STORED,
    ...
    end_date TIMESTAMP WITH TIME ZONE GENERATED ALWAYS AS (
        start_date + INTERVAL '1 month' * duration_months
    ) STORED,
    ...
    completion_percentage DECIMAL(5,2) GENERATED ALWAYS AS (
        CASE 
            WHEN total_deposits_required > 0 THEN 
                (completed_deposits::DECIMAL / total_deposits_required) * 100
            ELSE 0
        END
    ) STORED,
...
BEGIN
    ...
    IF NEW.type = 'deposit' AND NEW.savings_plan_id IS NOT NULL THEN
        ...
        SET 
            ...
            next_deposit_date = next_deposit_date + INTERVAL '1 day' * (
                SELECT frequency_days FROM public.savings_plans WHERE id = NEW.savings_plan_id
            ),
            updated_at = NOW()
        ...
        SET 
            ...
            updated_at = NOW()
        WHERE id = NEW.savings_plan_id 
        AND completed_deposits >= total_deposits_required
        AND status = 'active';
    END IF;
...
COMMENT ON COLUMN public.savings_plans.total_deposits_required IS 'Nombre total de dépôts requis (calculé automatiquement)';
COMMENT ON COLUMN public.savings_plans.completion_percentage IS 'Pourcentage de completion du plan (calculé automatiquement)';
