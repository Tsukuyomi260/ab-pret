...
REACT_APP_BACKEND_URL=http://localhost:5000`}
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <div className="text-sm text-red-700 space-y-1">
              <p>• URL Supabase: {process.env.REACT_APP_SUPABASE_URL ? '✅ Définie' : '❌ Manquante'}</p>
              <p>• Clé Supabase: {process.env.REACT_APP_SUPABASE_ANON_KEY ? '✅ Définie' : '❌ Manquante'}</p>
              <p>• Backend URL: {process.env.REACT_APP_BACKEND_URL ? '✅ Définie' : '❌ Manquante'}</p>
              <p>• Mode: {process.env.NODE_ENV || 'Non défini'}</p>
            </div>
            ...
            <button 
              onClick={() => {
                ...
                console.log('[CONFIG] Variables d\'environnement:', {
                  ...
                  url: process.env.REACT_APP_SUPABASE_URL ? 'Définie' : 'Manquante',
                  key: process.env.REACT_APP_SUPABASE_ANON_KEY ? 'Définie' : 'Manquante'
                });
            ...
            <button 
              onClick={() => {
                ...
                testAllConnections().then(result => {
                  console.log('[CONFIG] Résultat du test:', result);
                  alert(`Test de configuration: ${result.success ? 'SUCCÈS' : 'ÉCHEC'}\nDétails: ${result.error || result.message || 'Aucun détail'}`);
                }).catch(error => {
...
const App = () => {
  ...
  useEffect(() => {
    const checkConfiguration = () => {
      try {
        console.log('[APP] Vérification des variables d\'environnement...');
        ...
        console.log('[APP] Variables d\'environnement:', {
          ...
          url: process.env.REACT_APP_SUPABASE_URL ? 'Définie' : 'Manquante',
          key: process.env.REACT_APP_SUPABASE_ANON_KEY ? 'Définie' : 'Manquante',
          backend: process.env.REACT_APP_BACKEND_URL ? 'Définie' : 'Manquante'
        });
        
        // Configuration OK si toutes les variables sont présentes
